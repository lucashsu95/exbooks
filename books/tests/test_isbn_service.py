...
class TestLookupByISBN:
    ...
    def test_lookup_success(self, mock_get, mock_cache):
        """測試成功查詢書籍。"""
        # 設定 mock
    ...
    @patch("books.services.isbn_service.cache")
    @patch("books.services.isbn_service.httpx.get")
    def test_lookup_no_results(self, mock_get, mock_cache):
        """測試查無書籍。"""
        mock_cache.get.return_value = None
        mock_response = MagicMock()
    ...
    @patch("books.services.isbn_service.cache")
    def test_lookup_cached_result(self, mock_cache):
        """測試快取命中。"""
        cached_data = {
            "title": "快取書籍",
            "author": "快取作者",
    ...
    @patch("books.services.isbn_service.cache")
    @patch("books.services.isbn_service.httpx.get")
    def test_lookup_timeout(self, mock_get, mock_cache):
        """測試 API 超時。"""
    ...
    @patch("books.services.isbn_service.cache")
    @patch("books.services.isbn_service.httpx.get")
    def test_lookup_request_error(self, mock_get, mock_cache):
        """測試網路請求錯誤。"""
    ...
    @patch("books.services.isbn_service.cache")
    @patch("books.services.isbn_service.httpx.get")
    def test_lookup_unexpected_error(self, mock_get, mock_cache):
        """測試未預期的錯誤。"""
        mock_cache.get.return_value = None
    ...
    @patch("books.services.isbn_service.cache")
    @patch("books.services.isbn_service.httpx.get")
    def test_lookup_missing_fields(self, mock_get, mock_cache):
        """測試缺少某些欄位的回應。"""
        mock_cache.get.return_value = None
        mock_response = MagicMock()
        mock_response.json.return_value = {

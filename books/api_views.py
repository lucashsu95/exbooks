...
class SharedBookViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = SharedBook.objects.select_related("official_book", "owner__profile", "keeper__profile").all().order_by("-listed_at")
    serializer_class = SharedBookSerializer
    permission_classes = [IsAuthenticatedOrReadOnly]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    search_fields = ["official_book__title", "official_book__author", "condition_description"]
    ...
    def list_for_transfer(self, request, pk=None):
        ...
        try:
            ...
            return Response({"status": "Book listed for transfer successfully."}, status=status.HTTP_200_OK)
    ...
    def suspend(self, request, pk=None):
        ...
        try:
            ...
            return Response({"status": "Book suspended successfully."}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)

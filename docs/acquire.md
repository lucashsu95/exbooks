...
10. 年滿 **18 歲** 以上之成年人，方能註冊使用本系統及服務。
    > **實作現況：** `UserProfile` 已有 `birth_date` 欄位、`is_adult` 屬性及 `validators.py` 驗證器，但註冊流程尚未強制阻擋未滿 18 歲用戶。
...
> **實作現況：**「拒絕借出次數」、「貢獻書籍流通狀態統計數」、「持有書籍流通狀態統計數」已實作於公開個人資料頁面。信用積分（`trust_score`）、逾期次數（`overdue_count`）、成功歸還次數（`successful_returns`）已實作於 `UserProfile` 模型。
...
### 7.8 AI 聊天機器人
1. **Gemini API 整合**：實作 `ai` app，整合 Google Gemini API 提供 AI 書本推薦與問答服務。
2. **Server-Sent Events (SSE)**：使用 `ChatSSEView` 實作串流回應，支援打字機效果。
3. **HTMX Chat Widget**：前端輕量化 HTMX 聊天元件，無需全頁重新整理。
4. **對話快取**：使用 Redis 快取對話歷史（`ConversationCache`），支援連續對話上下文。

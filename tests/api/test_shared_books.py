...
class TestSharedBookAPI:
    def setup_method(self):
        ...
        self.user = User.objects.create_user(
            username="bookowner", password="password123", email="owner@example.com"
        )
        ...
        self.off_book = OfficialBook.objects.create(
            isbn="1112223334445", title="Shared Book Title", author="Shared Author"
        )
    ...
    def test_list_shared_books(self):
        ...
        if "results" in response.data:
            ...
            assert (
                response.data["results"][0]["official_book"]["title"]
                == "Shared Book Title"
            )
        else:
    ...
    def test_list_for_transfer_action(self):
        ...
        token_res = self.client.post(
            token_url, {"username": "bookowner", "password": "password123"}
        )
        token = token_res.data["access"]
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")

        url = reverse(
            "books:shared-list-for-transfer", kwargs={"pk": self.shared_book.pk}
        )
    ...
    def test_unauthorized_action(self):
        ...
        token_url = reverse("token_obtain_pair")
        token_res = self.client.post(
            token_url, {"username": "otheruser", "password": "password123"}
        )
        token = token_res.data["access"]
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
